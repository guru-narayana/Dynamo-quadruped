...
def point_matrix(P1,DC1,DC2,right = 1):
    P2 = np.array([right*(stepLength/2)*DC2-0.04, P1[1] ,right*(stepLength/2)*DC1])
        ...
        if jnt != []:
            if not halt and not abs(rotate):
                if swing:
                    ...
                    rf_end_point = np.array([-DC2*(stepLength/2) -0.04,RF[1],-DC1*(stepLength/2)])
                    print(rf_end_point,RF)
                    lb_end_point = np.array([DC2*(stepLength/2) -0.04,LB[1],DC1*(stepLength/2)])
                    ...
                    rb_end_point = np.array([-DC2*(stepLength/2)-0.04,RB[1],-DC1*(stepLength/2)])
                    lf_end_point = np.array([DC2*(stepLength/2)-0.04,LF[1],DC1*(stepLength/2)])
                    start = rospy.get_time()
                    t = 0
                    while (t < T):
                        t = rospy.get_time() - start
